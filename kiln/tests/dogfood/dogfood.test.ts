...
test("DOGFOOD complete: a full stub walk PASSES 001's kiln/validate/log.ts (R1–R6)", () => {
  const walk = buildStubWalk();
...
test("DOGFOOD prefix: a truncated stream still PASSES (the closed terminal reconstructs, F-RECON)", () => {
  const walk = buildStubWalk();
...
test("DOGFOOD forced-violation: a seq gap is NAMED by 001's validator (R2)", () => {
  const walk = buildStubWalk();
...
test("DOGFOOD negative (SC-002→SC-003): a broken no-silent-approval hole FAILs 001's validator", () => {
  const lines = buildStubWalk().lines.slice();
