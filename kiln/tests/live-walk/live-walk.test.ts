...
test("US1 SC-001 (F-LIVE-WALK): the live nine-gate walk PASSES 001's unmodified log.ts (R1–R6)", () => {
  const walk = buildLiveWalk({ mode: "live" }); // NC2: live default
...
test("US1 SC-001: every human-decided gate's completion resolves on a human decidedBy OR a distinct pre-delegation (never a silent approval)", () => {
  const walk = buildLiveWalk({ mode: "live" });
...
test("US1 SC-001: a truncated PREFIX of the live walk still reconstructs + PASSes log.ts", () => {
  const walk = buildLiveWalk({ mode: "live" });
...
test("US1 SC-002→SC-003 (P-V): a no-`decidedBy` broken walk FAILs log.ts with a named R3", () => {
  const broken = buildLiveWalk({ mode: "live", brokenNoDecider: true });
...
test("US1 SC-003 (F-SINGLE): a live walk's per-step snapshots are single-lane; a foundry throws", () => {
  const walk = buildLiveWalk({ mode: "live" });
...
test("US1 SC-004 (P-IV): the live walk realizes the affinity tax, live on wallClock", () => {
  const walk = buildLiveWalk({ mode: "live" });
...
test("US1 SC-006 (F-NOT-SILENT): the live walk's resident selection IS recorded (E3)", () => {
  const walk = buildLiveWalk({ mode: "live" });
  assert.ok(liveWalkRecordsSelection(walk, "live"), "the --live selection is recorded in the emitted log");
  const stub = buildLiveWalk({ mode: "stub" });
