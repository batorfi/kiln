...
test("US5 SC-006: OverlayCReady PASSES with the full overlay present/wired/deterministic/blocking/cloud-free", async () => {
   const r = await checkOverlayReady();
   ...
   assert.ok(/[Oo]verlaycready|PASS/.test(await reportOverlayReady()), "the report prints a passing OverlayCReady line");
});

// T020 (cont.) — falsifiability: removing/breaking exactly one element → a SPECIFIC named FAIL.
test("US5 SC-006: OverlayCReady is falsifiable — a broken Gate-0 auto-advance hole FAILs and names it", async () => {
   const broken = await checkOverlayReady({ brokenGate0: true });
...
test("US5 SC-006: a non-deterministic overlay render FAILs and names it (the broken-render hook)", async () => {
   const broken = await checkOverlayReady({ brokenRender: true });
