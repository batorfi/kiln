...
test("US3 SC-006 (F-LIVEREADY): LiveModelReady PASSES with the full live path wired", async () => {
  const r = await checkLiveModelReady();
  ...
  assert.ok(/PASS|LiveModelReady/.test(await reportLiveModelReady()), "reportLiveModelReady prints a passing line");
 });

// T014 (cont.): falsifiability — each hook opens exactly ONE hole, and LiveModelReady FAILs NAMING it.
test("US3 SC-006: --broken opens the no-`decidedBy` hole ⇒ FAIL, NAMED R3 (SC-002→SC-003)", async () => {
  const broken = await checkLiveModelReady({ brokenDogfood: true });
...
test("US3 SC-006: --broken-render makes the overlay non-deterministic ⇒ FAIL, NAMED (determinism)", async () => {
  const broken = await checkLiveModelReady({ brokenRender: true });
...
test("US3 SC-006: --broken-gate0 opens a Gate-0 auto-advance hole ⇒ FAIL, NAMED F1 (P-VI)", async () => {
  const broken = await checkLiveModelReady({ brokenGate0: true });
...
test("US3 SC-006 (F-NOT-SILENT): an unlogged --stub stand-in FAILs LiveModelReady, NAMED", async () => {
  const broken = await checkLiveModelReady({ stubUnlogged: true });
...
test("US3 SC-007: LiveModelReady runs no gate / admits no program (it is a probe, not a walk)", async () => {
  const r = await checkLiveModelReady();
  ...
  assert.ok(!(await brokenReady()), "a recorded --stub is accepted — only an UNLOGGED one fails");
});
async function brokenReady(): Promise<boolean> {
  return (await checkLiveModelReady({ stubUnlogged: false })).ready === false; // recorded stub ⇒ ready, not a failure
}
