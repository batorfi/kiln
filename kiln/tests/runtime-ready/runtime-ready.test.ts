...
test("US6 SC-006: the full runtime is RuntimeReady (wired + a valid, cloud-free emitted log)", async () => {
...
test("US6: reportRuntimeReady prints a human-readable READY block", async () => {
  const out = await reportRuntimeReady();
  assert.ok(out.startsWith("PASS"), "the report opens with PASS");
  assert.ok(out.includes("RuntimeReady"), "the report names the check");
});

test("US6 falsifiable (a): a missing/broken module FAILs and NAMES it", async () => {
  const r = await checkRuntimeReady({ writer: "/no/such/log-writer.ts" });
...
test("US6 falsifiable (a): the not-wired index FAILs", async () => {
  ...
  const r = await checkRuntimeReady({ index: tmp });
...
test("US6 falsifiable (b): a broken no-silent-approval hole FAILs the dogfood and names it", async () => {
  const r: RuntimeReadyResult = await checkRuntimeReady({ brokenDogfood: true });
